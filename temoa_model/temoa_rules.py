...
def LandLimit_Constraint(M, r, p, g):
    ...
    tech_to_group_dict = dict()
    for r, g, S_t in M.tech_groups.keys():
        if g not in tech_to_group_dict:
            tech_to_group_dict[g]=[]
        tech_to_group_dict[g].append(S_t)


    
    land_limit = M.LandLimit[r, p, g]
    ...
    land_cap_group = sum(
        M.V_Capacity[r, S_t, S_v] * M.LandPerCapacitywoPeriod[r, S_t, S_v]
        for S_t in tech_to_group_dict[g]
    ...
    land_act_group = sum(
        ...
        * M.V_FlowOut[r, S_p, S_s, S_d, S_i, t, S_v, S_o]
        for t in tech_to_group_dict[g]
        if (r, p, t) in M.processVintages.keys()
        for S_v in M.processVintages[r, p, t]
        if (r, t, S_v) in M.LandPerActivitywoPeriod.keys()
    ...
    expr = land_cap_group+land_act_group  <= land_limit
    return expr
